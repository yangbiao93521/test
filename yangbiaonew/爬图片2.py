...
#     f.write(img)
#
a=requests.get('http://desk.zol.com.cn/bizhi/7590_94219_2.html',headers={"User-Agent":user_agent[n]})
b=a.text
c=re.compile('<li class=".*">\s*<a href="(.*?)">')
e=re.findall(c,b)
# print(e)
f=[]
for   i  in  e:
    g="http://desk.zol.com.cn"+i
    f.append(g)
# print(f)
n=[]
for   i   in   f:
    h=requests.get(i)
    k=h.text
    l=re.compile('<a target="_blank" id="1920x1200" href="(.*?)">')
    m=re.findall(l,k)
    if   len(m):
        n.append(m)
# print(n)
t=[]
for   i   in    n:
    p=requests.get("http://desk.zol.com.cn"+i[0])
    s=p.content.decode("gb2312")
    w=re.compile('<img src="(.*?)">')
    r=re.findall(w,s)[0]
    t.append(r)
for   i  in  range(len(t)):
    u=requests.get(t[i])
    v=u.content
    with   open(f'{i}.jpg','wb')  as   x:
        x.write(v)
